...
def sendMessage(user_inst: User):
    r = requests.post(address + "sendMessage", 
                      params={"chat_id":user_inst.id,
                              "text":user_inst.text,
...
def createUser(id,username):
    data = {"id":id,"state":userState.BEGIN.value,"username":username,"result":False}
...
def ProcessMessage(msg):
    ...
    if(not msg["from"].get("last_name",None) is None):
