...
class UnsupportedTypesHandler(Handler):
    def can_handle(self, message: dict) -> bool:
        ...
        return not any(field in message[msg_type]
                       for field in supported_fields.get(msg_type, [])
                       )
    def handle(self, message:dict) -> bool:
        tg_handler.send_text(message_parser.get_id(message), "Sorry, I can't handle this message")
        return False